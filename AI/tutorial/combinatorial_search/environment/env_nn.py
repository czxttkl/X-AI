...
This environment simulates f(x_p, x_o, A_p, A_o) where x_o is not
considered (or considered as fixed)
"""
import numpy
import time
import math
...
class Environment:
    ...
    def all_possible_next_state_action(self, state_and_step):
        assert len(state_and_step.shape) == 1 and state_and_step.shape[0] == self.k + 1
    ...
    def test(self, TRIAL_SIZE, RANDOM_SEED, learn_step_counter, cpu_time, env_name, rl_model):
        ...
        rl_model.tb_write(tags=['Prioritized={0}, gamma={1}, seed={2}, env={3}/Test Ending Output'.
                      format(rl_model.prioritized, rl_model.gamma, RANDOM_SEED, env_name),
                            'Prioritized={0}, gamma={1}, seed={2}, env={3}/Test Ending Qvalue'.
                      format(rl_model.prioritized, rl_model.gamma, RANDOM_SEED, env_name),
                            ],
                      values=[test_output,
                              q_val],
